...
export class CreateProjectComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.updateFarm) {
      ...
      this.Description = this.updateFarm.district;
      this.PrerequisiteLearnings = this.updateFarm.tehsil;
      this.selectedImages = this.updateFarm.projectImages;
      this.selectedIndustryItem = this.updateFarm.industries.map(industry => ({
        id: industry.industryId, 
        name: industry.industryName
      }));

      this.selectedGradeItem = this.updateFarm.grades.map(industry => ({
        id: industry.gradeId, 
        name: industry.gradeName
      }));

      this.selectedRoleItem = this.updateFarm.roles.map(industry => ({
        id: industry.roleId, 
        name: industry.roleName
      }));
      this.selectedSubjectItem = this.updateFarm.subjects.map(industry => ({
        id: industry.subjectId, 
        name: industry.subjectName
      }));

      (this.updateFarm.weeks || []).forEach(val => {
  ...
  addMultiWeek(val = null): void {
    ...
    this.selectedToolItem = val?.tools?.map(tool => ({
      id: tool.toolId, 
      name: tool.toolName
    })) || [];
    ...
    this.weeksArray.push(new UntypedFormGroup({
      objectives: new UntypedFormControl(val?.weekObjective || '', [
        // Validators for objectives can be added here
        // Validators.maxLength(12),
      ]),
      lessonPlan: new UntypedFormControl(val?.weekLesson || '', [
        // Validators for lesson plan can be added here
        // Validators.maxLength(12),
      ]),
      weekId: new UntypedFormControl(val?.weekId || 0, [
      ...
      activities: new UntypedFormControl(val?.weekActivities || '', [
