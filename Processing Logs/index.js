...
async function getData() {
    ...
    let url = mydata.protocol + window.location.hostname + mydata.port + mydata.url;
    try {
        let res = await fetch(url);
        let JSONrep = res.json();
        document.getElementById('myTable').style.display = 'table';
        hideLoader();
        return await JSONrep;
    } catch (error) {
        console.log(error);
        document.getElementById('myTable').style.display = 'table';
        hideLoader();
...
const start = async function (renderOnly) {
    if(renderOnly){
...
class RdataTB {
    ...
    constructor(IdTable, Options = {
        ...
        this.StyleS();
        this.ConvertToJson();
        if(renderControls){this.paginateRender();}; 
        if(renderControls){this.Control();};       
        this.search();
        this.RenderToHTML();
        this.PaginateUpdate();
    ...
    Control() {
        ...
        <tr>
          <td style="width:100%;">
             ...
             <input id="SearchControl" class="form-control shadow-none" placeholder="Search" type="text" style="width:30%;float: right;">
          </td>
        </tr>
      </table>
        `;
        ...
        document.getElementById('x__FIRST__X').onclick = () => {
            this.firstItem();
            this.highlight(this.searchValue);
            this.DoHide();
        };
        document.getElementById('x__LAST__X').onclick = () => {
            this.lastItem();
            this.highlight(this.searchValue);
            this.DoHide();
        };
    ...
    paginateRender() {
        ...
        <a id="x__FIRST__X" style="cursor:pointer;user-select: none;">&lt;&lt;</a>
        <a id="x__PREV__X" style="cursor:pointer;user-select: none;">&lt;</a><div id="PF"></div>
        <a id="x__NEXT__X" style="cursor:pointer;user-select: none;">&gt;</a>
        <a id="x__LAST__X" style="cursor:pointer;user-select: none;;">&gt;&gt;</a></div>`;
    ...
    RenderToHTML(SlecTloaf = null) {
        ...
        for (let I = 0; I < this.HeaderDataTable.length; I++) {
            header += `<th style="cursor: pointer; font-size: 22px; border: none;" id="${this.HeaderDataTable[I]}_header" class="columns tablesorter-header">${this.HeaderDataTable[I]}</th>\n`;
            footer += `<th style="cursor: pointer; font-size: 22px; border: none;" id="${this.HeaderDataTable[I]}_footer" class="columns tablesorter-header">${this.HeaderDataTable[I]}</th>\n`;
        }
        ...
        if (SlecTloaf === null) {
            for (let ___row = 0; ___row < ifUndefinded; ___row++) {
                ...
                for (let ___cell = 0; ___cell < this.HeaderDataTable.length; ___cell++) {
                    ToCell += `<td style="border: none;font-size: 21px;" class="${this.HeaderDataTable[___cell]}__row">${this.DataToRender[___row][this.HeaderDataTable[___cell]]}</td>\n`;
                }
                row += `<tr>${ToCell}</tr>\n`;
            }
        }
            ...
            for (let ___row = 0; ___row < SlecTloaf.length; ___row++) {
                let ToCell = '';
                for (let ___cell = 0; ___cell < this.HeaderDataTable.length; ___cell++) {
                    ToCell += `<td style="border: none;font-size: 21px;" class="${this.HeaderDataTable[___cell]}__row">${SlecTloaf[___row][this.HeaderDataTable[___cell]]}</td>\n`;
                }
                row += `<tr>${ToCell}</tr>\n`;
            }
